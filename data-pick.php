...
<script type="text/javascript">
        ...
        function update_source_step1(graphid,name, sysname)
        {
...
//                        opener.document.forms["frmMain"].node_new_name.value = graphid;
                        opener.document.forms["frmMain"].node_new_name.value = sysname;
                        opener.document.forms["frmMain"].node_label.value = sysname;
                }
                self.close();
        }
        </script>
...
if($hosts->isNotEmpty()) {
        ...
        if($hosts->isNotEmpty())
        {
                ...
                foreach($hosts as $host) {
                        ...
                        $key = $host['id'];
                        $name = $host['name'];
                        $sysname = strtoupper($host['sysname']);
                        echo "<a href=\"#\" onclick=\"update_source_step1('$key','$name', '$sysname')\">". $host['name'] . "(" . strtoupper($host['sysname']) . ")</a>";
                        echo "</li>\n";
                        $i++;
                }
        }
